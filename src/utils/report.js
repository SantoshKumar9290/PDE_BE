		...
		let barcoderow = [
    {
		...
		if(reportType == "checkSlip"){
									...
									[	
										{
											text: [
												...
												{text: '\nDocument Nature: ', fontSize: 8, bold: true},{text:` ${regSubType}`,fontSize:8}
											]
										},
										{
											text:[
			...
			if(data?.executent && data?.executent?.length >0){
				checkSlip.content[1].table.body.push([{text:'Seller Details',alignment:'center',fillColor: '#50c4eb',
				color: '#343f42',}]);
				let sNo =0;
			...
			if(data?.executent && data?.executent?.length >0){
						...
						for(let j in data.executent[i].represent){
							...
							if(seNo === 0){
								a  = [
									...
									{text:`${toTitleCase(data.executent[i].represent[j].name)}${data?.executent[i]?.represent[j]?.aadhaar? `Aadhar: \n${maskAadhaar(data?.executent[i]?.represent[j]?.aadhaar.length>12 ? DecryptAdrwithPkcs(data?.executent[i]?.represent[j]?.aadhaar) :data?.executent[i]?.represent[j]?.aadhaar)}`:''}\nMobile No.: ${maskPhoneNumber(data?.executent[i]?.represent[j]?.phone)}`,fontSize:8},
				...
				// if(repArryEx && repArryEx[0].table  && repArryEx[0].table.body   && repArryEx[0].table.body.length >1){
					checkSlip.content[1].table.body.push([{text:'Seller Representative Details',alignment:'center',fillColor: '#50c4eb',
					color: '#343f42',}]);
			...
			if(data?.claimant && data?.claimant?.length >0){
				...
				//  if(repArryCl && repArryCl[0].table && repArryCl[0].table.body && repArryCl[0].table.body.length >1){
					checkSlip.content[1].table.body.push([{text:'Buyer Representative Details',alignment:'center',fillColor: '#50c4eb',
					color: '#343f42',}]);
			...
			if(data?.property && data.property.length >0){
				...
				if(chargeArry && chargeArry.length >0){
			...
			if(data.documentNature.TRAN_MAJ_CODE === "07"){
                ...
                const dateParts = wefValue.split('-');
                if (dateParts.length === 3) {
                  wefValue = `${dateParts[2]}-${dateParts[1]}-${dateParts[0]}`;
                 }
                }
			...
			let cdmaArry =  [
                {
                    style:'insideTable12',
                    table:{
                        widths:[20,45,60,85,40,35,35,35,30,30],
                        body:[
                            [
                                {text:'S.No.',fillColor: '#ede8fa',fontSize:8,bold:true,alignment:'center'},
                                {text:'PTIN Number',fillColor: '#ede8fa',fontSize:8,bold:true,alignment:'center'},
								{text:'Mutation Fee',fillColor: '#ede8fa',fontSize:8,bold:true,alignment:'center'},
                ...
                for(let j of data.property){
                    ...
                    if(cdmaData && cdmaData.propertyAddress!= undefined && cdmaData.propertyAddress!= ""){
                        const a = [
							...
							{text: [{ text: 'Mutation Fee Amount: ', bold: true },`${j?.payableMutationFee ?? 0}\n`,{ text: 'Property Tax Amount: ', bold: true },`${response?.mutationFee ?? 0}`], fontSize: 7},
           ...
           if (loginDetails.emplName) {
			  const currentDate = formatDate(' ', true);   // DD/MM/YYYY
              const currentTime = z(new Date().getHours()) + ":" + z(new Date().getMinutes()) + ":" + z(new Date().getSeconds());
			checkSlip.content.push({columns: [
            {
                text: "",
                width: 5,
            ...
            {
                 text: `CheckSlip Generated By:- ${loginDetails.emplName} && CheckSlip Generated On:-  (${currentDate} & ${currentTime})`,
                fontSize: 8,
                alignment: 'left',
                margin: [30, 5, 0, 0]  
            }
			...
			if(data.documentNature.TRAN_MAJ_CODE ==" "){
				...
				if(TRAN_DESC ){
					...
					if(data?.property && data.property.length >0){
						for(let i in data?.property){
							...
							if(naturetype =="Sale"){
								...
								pdeStyle.content.push({text:`The Releasor hereby convey, transfer and assign all his.......... share of rights, title and interest together with all liberties, advantages, appurtenances and rights of easements in
					...
					if(naturetype == "Mortgage"){
						...
						pdeStyle.content.push({text:`a)	That the Mortgaged property is free from all charges and encumbrances whatsoever.
