...
const { EncryptAdrwithPkcs,encryptUID } = require("../utils");
...
class CCServices {
    ...
    WHERE m.sr_code = :SRCODE AND m.rdoct_no = :DOCNO AND m.ryear = :RYEAR AND m.book_no = :BOOKNO`;
     ...
     const partyQuery = `SELECT Code AS PartyCode,
      ...
      NVL(CASE WHEN LENGTH(aadhar) = 12 THEN SUBSTR(aadhar, 1, 1) || 'XXXXXXX' || SUBSTR(aadhar, 9, 4) END,
          NVL((SELECT SUBSTR(b.aadhar, 1, 1) || 'XXXXXXX' || SUBSTR(b.aadhar, 9, 4) FROM tran_ec_firms b WHERE b.sr_code = e.sr_code AND b.book_no = e.book_no AND b.doct_no = e.doct_no AND b.reg_year = e.reg_year AND b.code = e.code AND b.EC_NUMBER = e.EC_NUMBER AND ROWNUM = 1), 'NA')) AS UIDNO
      FROM tran_ec e
      ...
      ORDER BY partycode`;
            ...
            let scheduleQuery;
            scheduleQuery = `SELECT TD.SCHEDULE_NO, SUBSTR(LPAD(TD.VILLAGE_CODE, 7, '0'), 1, 2) AS SURVEYNODISTCD, SUBSTR(LPAD(TD.VILLAGE_CODE, 7, '0'), 3, 2) AS SURVEYNOMANDCD,
            ...
            if (DocDetails && DocDetails.length > 0) {
                try {
                    ...
                    let REGDOCUMENTLINK = `${process.env.PDE_UI}/CCdownloadPage?data=${ccData}`;
                    DocDetails = {
                        ...DocDetails[0],
                        DOCTYPE: doct_type,
            ...
            response.forEach(row => {
                ...
                if (!groupedData[key]) {
                    groupedData[key] = {
                        ...
                        UID: row.AADHAR.length == 12 ?encryptUID(String(row.AADHAR)) : row.AADHAR,
                        PARTIES: [],
                        WARD_NO: row.WARD_NO,
...
JOIN card.mst_revdist f 
    ON SUBSTR(e.webland_code, 1, 2) = f.district_code
JOIN card.mst_revmandal g 
    ON SUBSTR(e.webland_code, 1, 2) = g.district_code
    AND SUBSTR(e.webland_code, 3, 2) = g.mandal_code
WHERE
    h.hab_code = nvl(:village_code,e.hab_code)
...
GROUP BY
    ...
    h.village_name
ORDER BY
    f.district_name,g.mandal_name,h.village_name`;
