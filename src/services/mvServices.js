  ...
  mvAssitanceReport = async (reqData) => {
    try {
      ...
      const currentYear = new Date().getFullYear();
        ...
        if (!fs.existsSync(mvAssitDirectiory)) {
          fs.mkdirSync(mvAssitDirectiory, { recursive: true });
        }
      ...
      ESIGN_STATUS) values(${reqData.SR_CODE},1,${reqData.REQ_NO},'','',sysdate,'${response1[i].AADHAR}','','${response1[i].NAME}',${currentYear},'P')`;
            ...
            let insertresponse = await this.orDao.oDbInsertDocs(insertquery);
        }
        // const base64Pdf = pdfBuffer.toString('base64');
        return base64Pdf;
    } catch (ex) {
    ...
    try {
      ...
      if (response.length > 0) {
        response.forEach(item => {
          item.AADHAR = item.AADHAR.length > 12 ? DecryptAdrwithPkcs(item.AADHAR) : item.AADHAR;
        });
      }
      return response
    }
