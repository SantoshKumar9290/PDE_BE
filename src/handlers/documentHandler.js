...
class DocumentHandler {
    ...
    verifyStockHoldingId = async (req, res) => {
        ...
        let urlValue;
        let data;
        urlValue = `${process.env.CARD_API}/stock/getStockHoldingReceipt/stock`;
        data=`?certificateId=${reqBody.stockid}&certIssueDate=${reqBody.date}`;
        urlValue = urlValue+data;
		try {
			var config = {
				method: 'get',
				url: urlValue,
            ...
            if(response.data && response.data.status == true){
                ...
                if(stampDeficitRespData.CertificatesDetails!=undefined && stampDeficitRespData.CertificatesDetails){
                    ...
                    throw new PDEError({ err: "Payment details not found."});
                }
            }
        } catch (ex) {
            if(ex.response && ex.response.data?.status==false){
                res.status(400).send(ex.response.data);
            }
            else{
                return {
                    status: false,
                    message: ex?.message || "Invalid Stock Holding Id."
                }
            }
        }
	}
